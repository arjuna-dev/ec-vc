...
- If the user reports a bug, use the repo-local skill `$save-bug-report` at `.codex/skills/save-bug-report` to turn it into a Markdown bug report and save it in the repo. If the bug involves the file ingestion system, make sure the report includes which PDF pitch deck was used when the bug appeared.
