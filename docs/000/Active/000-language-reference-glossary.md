...
| Base L1 Structure | The shared starter contract every normal `L1` should begin from before any entity-specific extension is added. It includes shared `System` parameters, the shared `KDB` linkage section, and shared `General` parameters. | File Steward Contract |
...
| Loaded At Birth | The rule that a new normal `L1` should be born with its reciprocal KDB bridge layer already decided and declared as part of bootstrap, rather than relying on manual back-wiring later. | Product Reference |
