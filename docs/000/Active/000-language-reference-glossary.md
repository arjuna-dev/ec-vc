...
| Access Assignment | The scoped rule that links a user/contact to a project or scope, a role, a status, and access limits. `Access_Assignments` should become its own canonical `File` because access must be browsed, audited, and governed directly. | Files |
...
| File Shell | The shared shell renderer for file-level `File` collection surfaces. It should stay fixed while route-owned `File` payloads swap underneath it. | Product Reference |
...
| BB File | `Building Blocks File`. A special `System-Level File` that governs reusable UI building blocks, design primitives, shell parts, and reconstruction guidance. It is not a standard app-data `File` and is a controlled exception to the normal `System` / `LDB` subsection baseline. | Record Architecture |
...
| Record Shell | The approved shared shell renderer for `Record View`. Its purpose is to present a trustworthy snapshot of existing record data so the user can read, inspect, and verify the current record state. It should receive the selected record as payload input instead of forking by entity. It must not become the primary intake or record-formation surface. | Record Architecture |
| Add/Edit Record Shell | The approved shared shell renderer for record creation and editing. Its purpose is to form record data, guide active user input, and host intake-linked creation/edit behavior. It is the process surface where record formation begins, not the stable snapshot surface for existing record truth. | Intake Architecture |
| Fork Shell | The independent route-owned shared shell for branch-choice create flows. It may consume branch metadata from a source `File`, but it must not collapse into entity-specific page UI or be treated as an `Opportunities` helper. | Record Architecture |
| Shared Dialog Shell | The rule that the create / edit dialog is itself a shell surface. Any launching shell must pass the same canonical payload contract into it rather than reshaping grouped `View` structure locally. | Record Architecture |
| Shared Shell Header | The common top shell row that owns the `File` select menu for shell routes. Shell pages should use this shared header instead of adding a second local select or launch row underneath it. | Product Reference |
| Strict Feeder Chain | The governed shared traffic path for shared shell or surface rendering. Shells provide canonical section data, one feeder classifies and filters it, one builder normalizes it, and one renderer displays it. This is both an architecture rule and a bug-filter rule. | Intake Governance |
| Two-Layer Asset Rule | The rule that important shared system assets may need both a runtime utility layer and a BB or system-facing asset layer. This keeps naming, inspection, governance, backend logic, feeders, and frontend usage converged around the same approved unit. | Intake Governance |
| Governed System Asset | A shared asset important enough to need approved system identity, not only hidden runtime implementation. These assets should be nameable, inspectable, and governable across backend, feeder/translator, and frontend layers. | Intake Governance |
| User-Preference Asset | A shared asset that primarily serves user-facing customization or design foundations, such as base colors, text style, and other settings-layer controls. These should converge through design-system and settings language rather than through hidden page-local styling. | Intake Governance |
| Dialog Shell Route | The dedicated route-owned surface for the shared create / edit shell. It may switch `File` intentionally, but normal add/edit launches should route into it with explicit source context instead of improvising page-local dialog flows. | Record Architecture |
...
| Fixed Shell Styling | The rule that shell visuals stay fixed at shell level. `File` may change payload, labels, section membership, and explicitly approved capabilities, but should not silently restyle the shell. | Product Reference |
| Shared Middle-Field Selection | The rule that the selected middle fields for a given `File` are remembered once and reused by both `File Shell` cards and `Record Shell` hero rows. | Product Reference |
| Business Overview | A view label used in `Companies` for overview-style sections. No grouped subdivision layer is active. | Record Architecture |
| Grouped View (Deprecated) | A retired concept. Views should not subdivide into extra grouping layers. | Record Architecture |
| Record View | The eye-opened detailed surface for one selected record. Route-level `record-view` should now resolve into `Record Shell`. Its job is to show current record truth clearly, not to carry the primary intake workflow. | Product Reference |
...
| View Toolbar | The section bar below the hero/dashboard that renders the selected `File`'s view groups. It places normal sections on the left and `LDB` / `System` on the right. | Product Reference |
...
| L0 Events File View | The route-owned `File View` for the `L0` event layer. It uses the same shared `File` shell as the other file surfaces, but it is view-oriented and routes rows into the event log surface rather than a normal databook record page. | ECS Workstream Tracker |
| File | The entity/file level in canonical structure. Selecting a `File` chooses the entity payload source for a shell. | Record Architecture |
| Base File Structure | The shared starter contract every normal `File` should begin from before any entity-specific extension is added. It includes shared `System` parameters, the shared `LDB` linkage section, and shared `General` parameters. | File Steward Contract |
| File Bootstrap | The required full implementation scope for a new `File`: canonical entity, route, registry, shell, navigation, runtime ownership, default subsections, reciprocal LDB propagation, and working add/edit/create flows. A new `File` is not complete if it is only visible in the UI, and this bootstrap work is `Owner`-only authority. | Record Architecture |
| System-Level File | A special file that supports the product system itself rather than normal app-data operations. A system-level file may have an intentionally narrower approved subsection contract when the normal operational `File` baseline would not be useful. | Product Reference |
| View | The subsection grouping level in canonical structure, such as `System`, `General`, `Business Overview`, or `LDB`. | Record Architecture |
| Shared System Parameter | A canonical field in the shared `System` starter set that should keep the same meaning and naming across normal `File` entries. The current shared set is `ID`, `Creator`, `Datetime`, and `EventLog`. | File Steward Contract |
| Shared General Parameter | A canonical field in the shared `General` starter set that should keep the same meaning and naming across normal `File` entries. The current shared set is `Name` and `Summary`. | File Steward Contract |
| Shared LDB Base | The fact that every normal `File` should own an `LDB` subsection as its linkage container. This does not mean every `File` must use one identical list of relationship leaf names. | File Steward Contract |
| LDB Bridge Layer | The reciprocal relationship wiring that makes declared LDB links actually function between `File`s. It includes the approved owner path, reverse-read path, and the bridge choice between a dedicated join table and the shared `LDB_Relationships` owner path. | Record Architecture |
...
| Loaded At Birth | The rule that a new normal `File` should be born with its reciprocal LDB bridge layer already decided and declared as part of bootstrap, rather than relying on manual back-wiring later. | Product Reference |
...
| Entity Extension Layer | The independent structure a specific `File` adds after the shared base, including its own extra `View`s. | File Steward Contract |
...
| Token Alias Contract | The explicit runtime alias mapping declared on a `Token` when the live payload field name differs from the canonical token name. This is a shell-level agreement, not a page-level patch. | Record Architecture |
...
| LDB | A reusable reference or processing file surface that may not be golden-tier in product importance, but still behaves like an `File` in the shell and contract system. | Companion |
...
The structural entity name used by canon and shell ownership logic. This name is used to define `File`, `View`, and `Token` structure, but it is not automatically the same thing as the databook table name or route key.
