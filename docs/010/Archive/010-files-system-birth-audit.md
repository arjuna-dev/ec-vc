...
| File guide index | The file-guide map should track active/draft guide status. | `docs/100/100-File_Guides_Index.md` exists and lists `Files`, `System Files`, and draft `Access Assignments`. | `yes` | File Steward: index exists but is manually maintained. |
| Registry entry | `Files` should be accepted as the visible System Files file. | `structureRegistry` defines `Files` with key `file-system`, label `System Files`, route `/file-system`, and `showInWorkspaceNav: true`. | `yes` | Architect Steward: accepted through registry. |
...
The drawer now consumes the registry export for both main workspace files and `LDB` files.
