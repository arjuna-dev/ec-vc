...
This document describes how Games should operate in practice.

It sits between the root Games rules in `docs/003` and the concrete Game Rulebook in `docs/300`.

## Operating Rule

Game operation should convert approved structure into visible guidance.
...
Current first-pass operation:

1. A file, record, task, or relationship exposes an approved objective.
2. The game layer translates that objective into a visible quest, board position, or priority.
...
The `Game Steward` explains and governs this operating layer.
