...
| Term | Meaning |
| --- | --- |
| `System Files` | The app-facing file-definition surface backed by the `Files` registry. |
| `Files` | The canonical registry entity that stores file-definition records. |
| `File Guide` | The `.md` guide that explains and governs a specific file; active guides live in `docs/100`, archive memory lives in `docs/100/Archive`. |
| `File Class` | The declared structural class for a file row or surface, such as `File`, `View`, or another approved class. |
| `Ownership Mode` | The declared ownership rule for the file or governed field path. |
| `Steward` | The governing role or guide responsible for keeping a file honest and aligned with canon. |
| `System` | The shared system/provenance/runtime-control section. |
| `General` | The shared human-readable identity and description section. |
| `LDB` | The shared relationship section that connects a file to governed local data and other files. |
