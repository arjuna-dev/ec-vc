...
- file name: `Files`
- file guide: `docs/100/b3. Files.md`
...
This guide is different from `docs/000/c. System.md`.

- `docs/000/c. System.md` is the parent rule and architecture authority for file guides.
- `docs/100/b3. Files.md` is the guide for the actual `Files` canonical entity and registry table.
- `docs/100/a1. System.md` is the guide for the app-facing `System Files` surface backed by `Files`.
...
| `File Guide` | The `.md` guide attached to a file record. Active file guides live in `docs/100`; archived guide memory lives in `docs/100/Archive`. |
| `sourceKey` | The stable file identity used to connect a file row to the file/page registry and runtime shell path. |
| `File Class` | The declared structural class for the file row, such as `File`, `View`, or another approved class. |
| `File Order` | A registry/bootstrap ordering field. It should not be treated as broad structure authority unless an approved contract says it is. |
| `Ownership Mode` | The declared ownership model for the file, such as `root_owned`. |
| `Defined Structure` | The declared section/view/token structure for the file. |

## Audit Vocabulary

`Files` should help the `Owner` and the `File Steward` understand file behavior using three audit labels:

- `Direct Path`
- `Fallback / Safety Path`
- `Override Path`

These labels should make it possible to tell whether a file row is being governed through true owned structure or through hidden repair logic.

### Direct Path

For `Files`, the `Direct Path` is:

- the stored `Files` row
- the row's owned `Defined_Structure`
- runtime hydration from that stored row
- shared shell rendering from that same hydrated structure

If a file cannot be explained through that direct chain, the file is not yet fully trustworthy.

### Fallback / Safety Path

For `Files`, `Fallback / Safety Path` includes things like:

- static file registries that behave like second authority
- default-file helpers that silently recreate or rewrite rows
- normalization passes that keep adjusting live values
- guessed structure or guessed ownership when a row is incomplete

These should be named explicitly, not hidden behind the appearance of a working file system.

### Override Path

For `Files`, `Override Path` includes things like:

- local state shadowing stored file rows
- page-level remapping that competes with canonical file structure
- local storage overrides or manual layer-specific exceptions

If an override exists, the steward should be able to say:
...
- Do not confuse the parent guide `docs/000/c. System.md` with the `Files` registry entity.
- Do not create a file row without a clear `sourceKey`, class, owner, steward, guide path, and structure declaration.
- Do not treat file visibility as equivalent to file birth.
- Do not treat a guide path as proof that the file is fully born.
- Use the file birth checklist to decide whether a file is active truth, partially born, parked direction, or not born.
...
The `File Steward` should also be able to record for each file row:

- its `Direct Path`
- its `Fallback / Safety Path`
- its `Override Path`

If that record cannot be stated clearly, the file row is not yet fully understood.
...
- file guide exists: `yes`, `docs/100/b3. Files.md`
