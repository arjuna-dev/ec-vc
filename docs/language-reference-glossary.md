...
| Base L1 Structure | The shared starter contract every normal `L1` should begin from before any entity-specific extension is added. It includes the shared `System`, `KDB`, and `General` baseline. | File Steward Contract |
...
| Shared System Token | A canonical field in the shared `System` starter set that should keep the same meaning and naming across normal `L1` files. | File Steward Contract |
| Shared KDB Base | The common starter relationship set that new normal `L1` files should inherit so they can link cleanly with the rest of the file system from birth. | File Steward Contract |
