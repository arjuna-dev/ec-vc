<template>
  <div class="add-edit-bb-shell-window">
    <div class="add-edit-bb-shell-window__frame" :style="frameStyle">
      <div class="add-edit-bb-shell-window__header-block">
        ...
        />
        <div class="add-edit-bb-shell-window__divider" />
      </div>

      <div class="add-edit-bb-shell-window__body">
        <div class="add-edit-bb-shell-window__render-row">
          <div class="add-edit-bb-shell-window__render-main">
            <BbRenderFrame>
              <BuildingBlockPreviewTile
                v-if="selectedBlockKey"
                :block-key="selectedBlockKey"
                class="add-edit-bb-shell-window__rendered-bb"
              />
            </BbRenderFrame>
          </div>
        </div>
        <div class="add-edit-bb-shell-window__divider add-edit-bb-shell-window__divider--body" />
        <div class="add-edit-bb-shell-window__tabs-row">
          <SectionTabs
            v-model="activeSectionTab"
            :left-tabs="sectionTabsLeft"
            :right-tabs="[]"
          />
          <div class="add-edit-bb-shell-window__tabs-actions">
            <MainMenuIconButton
            ...
            <button
              type="button"
              class="add-edit-bb-shell-window__grid-toggle"
            ...
            <button
              type="button"
              class="add-edit-bb-shell-window__close"
        ...
        <BbSelectionFrame v-if="bbGridExpanded">
          <div class="add-edit-bb-shell-window__bb-scroll ds-mini-scrollbar">
            <div
              v-if="detailsPanelOpen && layeredBbColumns.length"
              class="add-edit-bb-shell-window__bb-layer-grid"
              :style="{ '--bb-layer-column-count': String(layeredBbColumns.length) }"
            >
              <section
                v-for="column in layeredBbColumns"
                :key="column.layer"
                class="add-edit-bb-shell-window__bb-layer-column"
              >
                <div class="add-edit-bb-shell-window__bb-layer-heading">{{ column.label }}</div>
                <div class="add-edit-bb-shell-window__bb-layer-list ds-mini-scrollbar">
                  <button
                    v-for="item in column.items"
                    :key="item.key"
                    type="button"
                    class="add-edit-bb-shell-window__bb-code"
            ...
            <div v-else-if="activeBbCodeItems.length" class="add-edit-bb-shell-window__bb-grid">
              <button
            ...
            <div v-else class="add-edit-bb-shell-window__bb-empty">
...
defineOptions({ name: 'AddEditBbShellWindow' })
...
<style scoped>
.add-edit-bb-shell-window {
  min-height: 100%;
  padding: 16px;
}

.add-edit-bb-shell-window__frame {
  min-width: 0;
  margin: 0 auto;
  padding: 16px;
...
.add-edit-bb-shell-window__close {
  display: inline-flex;
  align-items: center;
...
.add-edit-bb-shell-window__header-block {
  display: grid;
  gap: 10px;
  width: 100%;
}

.add-edit-bb-shell-window__divider {
  width: 100%;
  height: 1px;
  background: var(--ds-color-brand-light-grey);
}

.add-edit-bb-shell-window__body {
  display: flex;
...
.add-edit-bb-shell-window__render-row {
  display: grid;
  width: 100%;
}

.add-edit-bb-shell-window__render-main {
  display: grid;
  min-width: 0;
  width: 100%;
}

.add-edit-bb-shell-window__divider--body {
  margin: 0;
}

.add-edit-bb-shell-window__rendered-bb {
  width: 100%;
}

.add-edit-bb-shell-window__rendered-bb :deep(.building-block-tile-header) {
  display: none;
}

.add-edit-bb-shell-window__rendered-bb :deep(.building-block-preview-tile) {
  width: 100%;
  border: 0;
  box-shadow: none;
  background: transparent;
...
.add-edit-bb-shell-window__rendered-bb :deep(.building-block-preview-tile__stage) {
  width: 100%;
  padding: 0;
}

.add-edit-bb-shell-window__tabs-row {
  display: flex;
  align-items: center;
  gap: 12px;
  width: 100%;
...
.add-edit-bb-shell-window__tabs-actions {
  display: inline-flex;
  align-items: center;
  gap: 12px;
  margin-left: auto;
...
.add-edit-bb-shell-window__grid-toggle {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  width: 24px;
...
.add-edit-bb-shell-window__bb-grid {
  display: grid;
  grid-template-columns: repeat(3, minmax(0, 1fr));
  gap: 10px 12px;
}

.add-edit-bb-shell-window__bb-layer-grid {
  display: grid;
...
.add-edit-bb-shell-window__bb-layer-column {
  display: grid;
  align-content: start;
  gap: 8px;
  min-width: 140px;
...
.add-edit-bb-shell-window__bb-layer-heading {
...
.add-edit-bb-shell-window__bb-layer-list {
  display: grid;
  align-content: start;
  gap: 8px;
...
.add-edit-bb-shell-window__bb-scroll {
  max-height: 220px;
  overflow-y: auto;
  overflow-x: auto;
}

.add-edit-bb-shell-window__bb-code {
  min-width: 0;
  padding: 0;
  color: var(--ds-color-brand-dark-grey);
  background: transparent;
...
.add-edit-bb-shell-window__bb-empty {
  color: var(--ds-color-brand-dark-grey);
  font-family: var(--ds-font-body);
  font-size: var(--ds-font-size-sm);
  line-height: 1.4;
