            ...
            <div v-for="row in displayRows" :key="row.id" class="col-12 col-sm-6 col-lg-4">
              <q-card flat bordered class="contact-card full-height">
                ...
                <q-card-section class="q-gutter-sm">
                  <div v-if="primaryEmail(row)" class="contact-card__field">
                    <q-icon name="mail" size="16px" class="q-mr-sm text-grey-7" />
                    <span>{{ primaryEmail(row) }}</span>
                  </div>
                  <div v-if="secondaryEmail(row)" class="contact-card__field">
                    <q-icon name="alternate_email" size="16px" class="q-mr-sm text-grey-7" />
                    <span>{{ secondaryEmail(row) }}</span>
                  </div>
                  <div v-if="row.Phone" class="contact-card__field">
                    <q-icon name="call" size="16px" class="q-mr-sm text-grey-7" />
                    <span>{{ row.Phone }}</span>
                  </div>
                  <div v-if="row.Country_based" class="contact-card__field">
                    <q-icon name="public" size="16px" class="q-mr-sm text-grey-7" />
                    <span>{{ row.Country_based }}</span>
                  </div>
...
function primaryEmail(row) {
  return String(row?.Professional_Email || row?.Personal_Email || '').trim()
}

function secondaryEmail(row) {
  const professionalEmail = String(row?.Professional_Email || '').trim()
  const personalEmail = String(row?.Personal_Email || '').trim()
  if (professionalEmail && personalEmail) return personalEmail
  return ''
}
...
function buildAvatarImage(row) {
  const label = String(row?.Name || row?.Professional_Email || row?.Personal_Email || 'Contact').trim()
  const initials =
    label
      .split(/\s+/)
      .filter(Boolean)
